...
import { formatQuestionDifficulty } from "@/lib/formatQuestionDifficulty"
...
export function NewQuestionClientPage({
  ...
  const {
    ...
    onFinish: async () => {
      try {
        const id = await fetchLatestQuestionId()
        setQuestionId(id)
      } catch (error) {
        console.error(error)
        errorToast("Failed to fetch the saved question ID.")
      }
  ...
  async function fetchLatestQuestionId() {
    const response = await fetch(
      `/api/questions/latest?jobInfoId=${encodeURIComponent(jobInfo.id)}`
    )

    if (!response.ok) {
      throw new Error("Unable to fetch latest question")
    }

    const json = await response.json()
    return z
      .object({ questionId: z.string() })
      .parse(json).questionId
  }
